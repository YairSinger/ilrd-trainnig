...
{
    size_t *arr_runner = arr;
    size_t *end_address = arr + size;

    printf("[ ");
    while (arr_runner < end_address)
    {
        printf("%u ", **(int **)arr_runner);
        ++arr_runner;
    }
    printf("]\n");
}*/
...
/***************************************************************/
	heap = HeapCreate(PriorityCmpFunc);
	if (NULL == heap)
	{
...
/***************************************************************/
	...
	for (i = 0; i < size; ++i)
	{
		...
		if(i + 1 != HeapGetSize(heap))
    	{
      		printf(BAD" EXPECTED: %lu RECIEVED: %lu "REG , i + 1, HeapGetSize(heap));
        	++error_count;
    	}
    	
    	if(control_data[i] != *(int*)HeapPeek(heap))
    	{
...
/***************************************************************/	
	...
	for (i = 0; i < size; ++i)
	{	
		if(remove_data[i] != *(int*)HeapPeek(heap))
    	{
...
{ 
	...
	size_t size = 7;
	int remove_param = 50;
	int control_remove = 50;
	int remove_param1 = 30;
	int control_remove1 = 30;
	void *remove = NULL;
	int x = 2;
...
/***************************************************************/
	...
	for (i = 0; i < size; ++i)
	{
    	...
    	if(control_data[0] != *(int*)HeapPeek(heap))
    	{
      		printf(BAD" EXPECTED: %d RECIEVED: %d "REG , control_data[i], 
      												*(int*)HeapPeek(heap));
        	++error_count;
...
/***************************************************************/	
	...
	remove = HeapRemove(heap, &remove_param, MatchFunc);
	...
	remove = HeapRemove(heap, &remove_param1, MatchFunc);
