...
#define NUM_OF_LETTERS 985084
#define NUM_OF_WORDS 104336
...
static size_t *my_strdup(const char *s) {
    ...
    return (size_t *)d;                           /* Return the new string*/
}


void TestMultiThreadCompSort()
{
	int fails = 0;
	int status = 0;
	size_t i = 0, j = 0;
	clock_t start, end;
	char word[100] = {'\0'};
	size_t *words[NUM_OF_WORDS] = {NULL};
	size_t len = 0;
    char *sort_func_name = "MultiThreadInsertionSort";
	...
	for (j = 0; j < NUM_OF_WORDS && 0 == status; ++j)
	{
    ...
    for (j = 0; j < 5; ++j)
    {
        for (i = 0; i < 3; ++i)
        {
            ...
            start = clock();
            status = MultiThreadsCompSort(words, NUM_OF_WORDS, threads[j]);
            end = clock();
            
            /*printArrayChar(words, NUM_OF_LETTERS); */       
            if (0 != status)
            { 
                ++fails_counter ;
            ...
            if (!IsArraySortedChar(words,NUM_OF_LETTERS))
            { 
                ++fails_counter ;
...
void SortingLinxDictLetters(int (*sort_func)(char arr[], size_t length, size_t num_of_threads), char *sort_func_name)
{
	...
	size_t fails_counter = 0;
	clock_t start, end;
    size_t dictionary_size = NUM_OF_COPIES * NUM_OF_LETTERS;
    char words[NUM_OF_COPIES * NUM_OF_LETTERS] = {'\0'};
    FILE *fp = fopen("/usr/share/dict/words", "r");

    for(l = 0, k = 0; l < NUM_OF_LETTERS; ++l)
    {
        fseek(fp, 0, SEEK_SET);
    ...
    for (j = 0; j < 5; ++j)
    {
        for (i = 0; i < 3; ++i)
        {
            ...
            start = clock();
            status = sort_func(words, NUM_OF_LETTERS, threads[j]);
            end = clock();
