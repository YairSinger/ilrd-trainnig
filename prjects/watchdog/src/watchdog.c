...
int WDStart(const char **path)
{
	...
	if ('1' == *(new_argv.new_argv[2])) /*forked by wd no need to fork again*/
	{	
		user_process_data.pid_for_signals = getppid();
	}
	else /*the process was forked by user*/
	{
...
void SetWDUtilsInitSemaphores(void) /***split two funcs */
{
	...
	user_process_data.exe_full_path = (const char *)new_argv.wd_path;
